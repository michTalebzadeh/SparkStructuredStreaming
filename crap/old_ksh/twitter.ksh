...
then
	create_assembly_sbt_file
        ...
        JAR_FILE="target/scala-2.10/${FILE_NAME}-assembly-1.0.jar"
	[ -f ${JAR_FILE} ] && rm -f ${JAR_FILE}
...
## in your code just do in main method

##    var outputDirectory  = args(0).toString
##    var numTweetsToCollect = args(2).toLong
##    var intervalSecs = args(3).toLong
##    var partitionsEachInterval = args(4).toInt

##
## check if the correct jar file exists
##
if [ ! -f ${JAR_FILE} ]
then
	mv -f target/scala-2.10/${FILE_NAME}-assembly-1.0.jar ${JAR_FILE}
fi
...
${SPARK_HOME}/bin/spark-submit \
                ...
                --jars /home/hduser/jars/spark-streaming-kafka-assembly_2.10-1.6.1.jar,/home/hduser/jars/utilities_2.10-0.1-SNAPSHOT.jar \
	        --class "com.databricks.apps.twitter_classifier.${FILE_NAME}" \
