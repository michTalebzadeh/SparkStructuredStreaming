#!/bin/ksh
#--------------------------------------------------------------------------------
#
# Procedure:    start hadoop daemons.ksh
#
# Description:  starts all hadoop nodes
#
# Parameters:   none
#
#--------------------------------------------------------------------------------
# Vers|  Date  | Who | DA | Description
#-----+--------+-----+----+-----------------------------------------------------
# 1.0 |04/03/16|  MT |    | Initial Version
#--------------------------------------------------------------------------------
#
ENVFILE=/home/hduser/dba/bin/environment.ksh

if [[ -f $ENVFILE ]]
then
        . $ENVFILE
        . ~/spark_1.5.2_bin-hadoop2.6.kshrc
else
        echo "Abort: $0 failed. No environment file ( $ENVFILE ) found"
        exit 1
fi

FILE_NAME=`basename $0 .ksh`
NOW="`date +%Y%m%d_%H%M`"
LOG_FILE=${LOGDIR}/${FILE_NAME}.log
[ -f ${LOG_FILE} ] && rm -f ${LOG_FILE}
print `date` ", Started $0" >> ${LOG_FILE}
cd ../${FILE_NAME}
sbt package
$SPARK_HOME/bin/spark-submit --packages amplab:succinct:0.1.5 --class "SimpleApp" --master local target/scala-2.10/simple-project_2.10-1.0.jar
print "" >> ${LOG_FILE}
print `date` ", Finished $0" >> ${LOG_FILE}
exit


